...
if __name__=='__main__':
    m=maze(10,10)
    m.CreateMaze(loadMaze=r'D:\Kuliah\Design And Analyst Of Algorithm\Nyoba1\DEMO.csv')

    searchPath,aPath,fwdPath=aStar(m)
    a=agent(m,footprints=False,color=COLOR.black,filled=True)
    b=agent(m,1,1,footprints=False,color=COLOR.black,filled=True,goal=(m.rows,m.cols))
    c=agent(m,footprints=False,color=COLOR.red)
